...
describe("artikelregistret", () => {
  test("innehåller exakt EXP#1 till EXP#31 utan duplicerade identiteter", async () => {
    const registry = await json("reflections/article-registry.json");
    assert.equal(registry.schemaVersion, 1);
    assert.equal(registry.articles.length, 31);
    ...
    assert.deepEqual(
      ...
      Array.from({ length: 31 }, (_, index) => `EXP#${index + 1}`)
    );
...
describe("skal och resursladdning", () => {
  test("alla 31 artikel-HTML laddar exakt en publik engagement-CSS och runtime", async () => {
    const registry = await json("reflections/article-registry.json");
    for (const article of registry.articles) {
      const source = await text(article.path);
